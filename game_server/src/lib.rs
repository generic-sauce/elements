...
const WAITING_FOR_PLAYERS_TIMEOUT_SECS: u32 = 5;
...
fn waiting_for_players(port: u16, domain_name: Option<&str>, identity_file: Option<&str>) -> (PeerManager, Vec<PeerHandle>, MasterToGameServerGoPacket) {
	...
	let mut timeout_counter = None; // the timeout starts when a MasterToGameServerGoPacket is received, or some client connects
	...
	for _ in TimedLoop::with_fps(JOIN_FPS) {
		if let Some(t) = &mut timeout_counter { *t += 1; }
		let evs = peer_manager.tick::<()>();

		for ev in evs {
			match ev {
				PeerEvent::NewPeer(_) => {
					if timeout_counter.is_none() { timeout_counter = Some(0); } // timeout starts here
		...
		if timeout_counter.unwrap_or(0) > WAITING_FOR_PLAYERS_TIMEOUT_SECS * JOIN_FPS {
